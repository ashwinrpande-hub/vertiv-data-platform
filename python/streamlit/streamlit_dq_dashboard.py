...
<style>
  ...
  .kpi-box {
    background:#1a2235; border-radius:10px; padding:18px;
    text-align:center; border:1px solid rgba(56,189,248,0.15);
  }
...
with st.sidebar:
    ...
    dims = st.multiselect(
        ...
        default=["COMPLETENESS","ACCURACY","CONSISTENCY","TIMELINESS","UNIQUENESS","VALIDITY"],
    )
    st.divider()
    st.markdown("**Data Products**")
    for dp in ["Sales Performance","Customer 360","Revenue Analytics","Opportunity Signals"]:
...
MOCK_KPI = {"TOTAL_BATCHES":24,"TOTAL_CHECKED":487320,
            "TOTAL_FAILED":842,"TOTAL_QUARANTINED":391,"AVG_PASS_RATE":99.27}
...
try:
    ...
    if not k.get("AVG_PASS_RATE"):
        k = MOCK_KPI
        st.info("ℹ️  No DQ log data yet — showing sample values. Run dq_framework.py to populate.")
except Exception:
    k = MOCK_KPI
...
kpis = [
    ...
    ("Avg Pass Rate",      f"{float(k.get('AVG_PASS_RATE',0)):.1f}%",
...
try:
    ...
    pivot = hdf.pivot(index="SOURCE_SYSTEM", columns="DAMA_DIMENSION", values="AVG_PASS_RATE")
except Exception:
    pivot = MOCK_HM
...
fig_hm = px.imshow(
    pivot,
    color_continuous_scale=[[0,"#f87171"],[0.5,"#fbbf24"],[0.95,"#fbbf24"],[1,"#34d399"]],
    zmin=85, zmax=100, text_auto=".1f", aspect="auto",
)
fig_hm.update_layout(
    paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
    font=dict(color="#e2e8f4"), height=280,
...
with lcol:
    ...
    try:
        ...
        tdf   = pd.DataFrame([
            {"RUN_DATE": d, "DAMA_DIMENSION": dim, "DAILY_PASS_RATE": 98 + np.random.rand()*2}
            for d in rng for dim in ["COMPLETENESS","VALIDITY","UNIQUENESS"]
        ])

    fig_t = px.line(tdf, x="RUN_DATE", y="DAILY_PASS_RATE",
                    color="DAMA_DIMENSION", markers=True)
    fig_t.add_hline(y=99, line_dash="dash", line_color="#34d399",
                    annotation_text="99% target", annotation_position="bottom right")
    fig_t.update_layout(paper_bgcolor="rgba(0,0,0,0)",
                        font=dict(color="#e2e8f4"), height=320,
...
with rcol:
    ...
    try:
        ...
        fdf = pd.DataFrame({
            "DAMA_DIMENSION":["ACCURACY","TIMELINESS","CONSISTENCY","VALIDITY","COMPLETENESS","UNIQUENESS"],
    ...
    if fdf["FAILURES"].sum() == 0:
        ...
        fig_f = px.bar(fdf, x="DAMA_DIMENSION", y="FAILURES",
                       color="DAMA_DIMENSION",
                       color_discrete_sequence=["#f87171","#fbbf24","#38bdf8",
                                                "#34d399","#c084fc","#2dd4bf"])
        fig_f.update_layout(paper_bgcolor="rgba(0,0,0,0)", showlegend=False,
                            font=dict(color="#e2e8f4"), height=320,
                            margin=dict(l=20,r=20,t=20,b=20))
...
dp_data = [
    {"Product":"Sales Performance",   "Owner":"Sales Ops",  "SLA":"15 min",
     "Status":"✅ HEALTHY", "Last Refresh":"2 min ago",  "30d Uptime":"99.8%", "DQ Score":"99.4%"},
    {"Product":"Customer 360",         "Owner":"CRM Team",   "SLA":"5 min",
     "Status":"✅ HEALTHY", "Last Refresh":"1 min ago",  "30d Uptime":"99.9%", "DQ Score":"99.7%"},
    {"Product":"Revenue Analytics",    "Owner":"Finance",    "SLA":"1 hour",
     "Status":"✅ HEALTHY", "Last Refresh":"42 min ago", "30d Uptime":"99.5%", "DQ Score":"99.1%"},
    {"Product":"Opportunity Signals",  "Owner":"RevOps",     "SLA":"2 min",
     "Status":"⚠️ LAGGING", "Last Refresh":"9 min ago",  "30d Uptime":"98.2%", "DQ Score":"97.8%"},
]
st.dataframe(
...
st.divider()
st.caption("Vertiv Data Platform  ·  Built on Snowflake  ·  DAMA 6-Dimension DQ Framework")
