...
GRANT USAGE ON SCHEMA VERTIV_RAW.SAP_EUROPE       TO ROLE VERTIV_SAP_EUROPE_READER;
GRANT USAGE ON SCHEMA VERTIV_RAW.JDE_AMERICAS     TO ROLE VERTIV_JDE_AMERICAS_READER;
GRANT USAGE ON SCHEMA VERTIV_RAW.QAD_EMEA         TO ROLE VERTIV_QAD_EMEA_READER;

-- ════════════════════════════════════════════════════════════════
-- SAP S/4HANA — EUROPE
-- ════════════════════════════════════════════════════════════════

CREATE OR REPLACE TABLE VERTIV_RAW.SAP_EUROPE.SALES_ORDERS (
...
CREATE OR REPLACE TABLE VERTIV_RAW.SAP_EUROPE.ORDER_ITEMS (
...
CREATE OR REPLACE TABLE VERTIV_RAW.SAP_EUROPE.CUSTOMERS (
...
CREATE OR REPLACE TABLE VERTIV_RAW.JDE_AMERICAS.SALES_ORDERS (
...
CREATE OR REPLACE TABLE VERTIV_RAW.JDE_AMERICAS.CUSTOMERS (
...
CREATE OR REPLACE TABLE VERTIV_RAW.QAD_EMEA.SALES_ORDERS (
...
CREATE OR REPLACE TABLE VERTIV_RAW.QAD_EMEA.CUSTOMERS (
...
CREATE OR REPLACE TABLE VERTIV_RAW.SALESFORCE_GLOBAL.OPPORTUNITIES (
...
CREATE OR REPLACE TABLE VERTIV_RAW.SALESFORCE_GLOBAL.ACCOUNTS (
...
CREATE OR REPLACE TABLE VERTIV_RAW.REALTIME.ORDER_EVENTS (
...
CREATE OR REPLACE STAGE VERTIV_RAW.STAGE.ERP_LANDING
  COMMENT = 'Internal stage for ERP batch CSV files. In prod: replace with external S3 stage.';

-- File formats
CREATE OR REPLACE FILE FORMAT VERTIV_RAW.STAGE.CSV_FORMAT
  TYPE                    = 'CSV'
  SKIP_HEADER             = 1
...
CREATE OR REPLACE FILE FORMAT VERTIV_RAW.STAGE.JSON_FORMAT
  TYPE                    = 'JSON'
  STRIP_OUTER_ARRAY       = TRUE
  ENABLE_OCTAL            = FALSE;

-- Snowpipe for SAP orders
CREATE OR REPLACE PIPE VERTIV_RAW.STAGE.PIPE_SAP_ORDERS
  AUTO_INGEST = FALSE    -- set TRUE + add SQS notification when S3 is connected
  COMMENT     = 'Snowpipe for SAP Europe sales orders CSV'
AS
COPY INTO VERTIV_RAW.SAP_EUROPE.SALES_ORDERS
...
FROM (
  ...
  FROM @VERTIV_RAW.STAGE.ERP_LANDING/sap_europe/orders/
)
...
CREATE OR REPLACE STREAM VERTIV_RAW.SAP_EUROPE.STR_SALES_ORDERS
  ON TABLE VERTIV_RAW.SAP_EUROPE.SALES_ORDERS
  APPEND_ONLY = TRUE
  COMMENT     = 'CDC stream → Silver DT_SILVER_ORDER_SAP';

CREATE OR REPLACE STREAM VERTIV_RAW.SAP_EUROPE.STR_CUSTOMERS
  ON TABLE VERTIV_RAW.SAP_EUROPE.CUSTOMERS
  APPEND_ONLY = TRUE;

CREATE OR REPLACE STREAM VERTIV_RAW.JDE_AMERICAS.STR_SALES_ORDERS
  ON TABLE VERTIV_RAW.JDE_AMERICAS.SALES_ORDERS
  APPEND_ONLY = TRUE;

CREATE OR REPLACE STREAM VERTIV_RAW.QAD_EMEA.STR_SALES_ORDERS
  ON TABLE VERTIV_RAW.QAD_EMEA.SALES_ORDERS
  APPEND_ONLY = TRUE;

CREATE OR REPLACE STREAM VERTIV_RAW.SALESFORCE_GLOBAL.STR_OPPORTUNITIES
  ON TABLE VERTIV_RAW.SALESFORCE_GLOBAL.OPPORTUNITIES
  APPEND_ONLY = TRUE;

CREATE OR REPLACE STREAM VERTIV_RAW.REALTIME.STR_ORDER_EVENTS
  ON TABLE VERTIV_RAW.REALTIME.ORDER_EVENTS
  APPEND_ONLY = TRUE;

-- ── TABLE GRANTS ──────────────────────────────────────────────
GRANT SELECT ON ALL TABLES IN SCHEMA VERTIV_RAW.SAP_EUROPE       TO ROLE VERTIV_PLATFORM_ADMIN;
GRANT SELECT ON ALL TABLES IN SCHEMA VERTIV_RAW.JDE_AMERICAS     TO ROLE VERTIV_PLATFORM_ADMIN;
GRANT SELECT ON ALL TABLES IN SCHEMA VERTIV_RAW.QAD_EMEA         TO ROLE VERTIV_PLATFORM_ADMIN;
GRANT SELECT ON ALL TABLES IN SCHEMA VERTIV_RAW.SALESFORCE_GLOBAL TO ROLE VERTIV_PLATFORM_ADMIN;
GRANT SELECT ON ALL TABLES IN SCHEMA VERTIV_RAW.REALTIME          TO ROLE VERTIV_PLATFORM_ADMIN;

-- Regional readers can see their schema's tables (row policy filters rows)
GRANT SELECT ON ALL TABLES IN SCHEMA VERTIV_RAW.SAP_EUROPE       TO ROLE VERTIV_SAP_EUROPE_READER;
GRANT SELECT ON ALL TABLES IN SCHEMA VERTIV_RAW.JDE_AMERICAS     TO ROLE VERTIV_JDE_AMERICAS_READER;
GRANT SELECT ON ALL TABLES IN SCHEMA VERTIV_RAW.QAD_EMEA         TO ROLE VERTIV_QAD_EMEA_READER;

-- ── VERIFY ────────────────────────────────────────────────────
SHOW TABLES IN DATABASE VERTIV_RAW;
SHOW STREAMS IN DATABASE VERTIV_RAW;
