...
CREATE OR REPLACE FUNCTION VERTIV_CURATED.UTILS.GENERATE_HASH_KEY(
  SRC_SYSTEM VARCHAR,
  ORIG_PK    VARCHAR
)
RETURNS VARCHAR(64)
...
CREATE OR REPLACE FUNCTION VERTIV_CURATED.UTILS.GENERATE_HASH_DIFF(
  ATTR_CONCAT VARCHAR
)
RETURNS VARCHAR(64)
LANGUAGE SQL
...
SELECT
  VERTIV_CURATED.UTILS.GENERATE_HASH_KEY('SAP', 'CUST-001')      AS HK_EXAMPLE,
  VERTIV_CURATED.UTILS.GENERATE_HASH_KEY('JDE', 'CUST-001')      AS HK_DIFFERENT_SOURCE,
  VERTIV_CURATED.UTILS.GENERATE_HASH_DIFF('Alice|Germany|EUR')   AS HD_EXAMPLE,
  LEN(VERTIV_CURATED.UTILS.GENERATE_HASH_KEY('SAP', 'TEST'))     AS KEY_LENGTH;
...
CREATE OR REPLACE TABLE VERTIV_CURATED.SALES.CUSTOMER (
...
CREATE OR REPLACE TABLE VERTIV_CURATED.SALES.PRODUCT (
...
COMMENT = '3NF Product master - SHA256 hash key - Vertiv product catalog';

-- ════════════════════════════════════════════════════════════════
-- SILVER: SALES ORDER HEADER
-- ════════════════════════════════════════════════════════════════

CREATE OR REPLACE TABLE VERTIV_CURATED.SALES.SALES_ORDER (
...
CREATE OR REPLACE TABLE VERTIV_CURATED.SALES.ORDER_LINE (
...
CREATE OR REPLACE TABLE VERTIV_CURATED.SALES.OPPORTUNITY (
...
CREATE OR REPLACE DYNAMIC TABLE VERTIV_CURATED.SALES.DT_SILVER_CUSTOMER_SAP
  TARGET_LAG = '5 minutes'
  WAREHOUSE  = TRANSFORM_WH
  COMMENT    = 'Auto-loads Silver CUSTOMER from SAP Bronze within 5 min lag'
AS
SELECT
  VERTIV_CURATED.UTILS.GENERATE_HASH_KEY('SAP', b.CUSTOMER_ID)  AS CUSTOMER_HK,
  ...
  TRUE                                                            AS IS_CURRENT,
  1                                                               AS RECORD_VERSION,
  VERTIV_CURATED.UTILS.GENERATE_HASH_DIFF(
...
FROM VERTIV_RAW.SAP_EUROPE.CUSTOMERS b
WHERE b.CUSTOMER_ID IS NOT NULL;  -- basic completeness gate

CREATE OR REPLACE DYNAMIC TABLE VERTIV_CURATED.SALES.DT_SILVER_ORDER_SAP
  TARGET_LAG = '5 minutes'
  WAREHOUSE  = TRANSFORM_WH
  COMMENT    = 'Auto-loads Silver SALES_ORDER from SAP Bronze within 5 min lag'
AS
SELECT
  VERTIV_CURATED.UTILS.GENERATE_HASH_KEY('SAP', b.ORDER_ID)       AS ORDER_HK,
  'SAP'                                                             AS SOURCE_SYSTEM,
  'EUROPE'                                                          AS SOURCE_REGION,
  ...
  b.NET_AMOUNT * COALESCE(
    (SELECT FX_TO_USD FROM VERTIV_CONFIG.REFERENCE.CURRENCY
  ...
  b.BATCH_ID                                                        AS SOURCE_BATCH_ID,
  CURRENT_TIMESTAMP()                                               AS LOAD_TIMESTAMP
FROM VERTIV_RAW.SAP_EUROPE.SALES_ORDERS b
WHERE b.ORDER_ID IS NOT NULL;

-- ════════════════════════════════════════════════════════════════
-- AUDIT TABLES (in VERTIV_AUDIT database)
-- ════════════════════════════════════════════════════════════════
...
CREATE OR REPLACE TABLE VERTIV_AUDIT.DQ.DQ_BATCH_LOG (
...
CREATE OR REPLACE TABLE VERTIV_AUDIT.DQ.REJECTED_RECORDS (
...
CREATE OR REPLACE TABLE VERTIV_AUDIT.DQ.QUARANTINE (
...
-- ── GRANTS ────────────────────────────────────────────────────
GRANT INSERT, SELECT ON ALL TABLES IN SCHEMA VERTIV_AUDIT.DQ      TO ROLE VERTIV_PLATFORM_ADMIN;
GRANT SELECT        ON ALL TABLES IN SCHEMA VERTIV_AUDIT.DQ        TO ROLE VERTIV_DATA_PRODUCT_OWNER;
GRANT SELECT ON TABLE VERTIV_CURATED.SALES.SALES_ORDER             TO ROLE VERTIV_SAP_EUROPE_READER;
GRANT SELECT ON TABLE VERTIV_CURATED.SALES.CUSTOMER                TO ROLE VERTIV_SAP_EUROPE_READER;
GRANT SELECT ON TABLE VERTIV_CURATED.SALES.SALES_ORDER             TO ROLE VERTIV_GLOBAL_ANALYST;
GRANT SELECT ON TABLE VERTIV_CURATED.SALES.CUSTOMER                TO ROLE VERTIV_GLOBAL_ANALYST;

-- ── VERIFY ────────────────────────────────────────────────────
SHOW TABLES        IN DATABASE VERTIV_CURATED;
SHOW DYNAMIC TABLES IN DATABASE VERTIV_CURATED;
SHOW TABLES        IN DATABASE VERTIV_AUDIT;
