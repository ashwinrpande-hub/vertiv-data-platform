...
CREATE OR REPLACE TABLE VERTIV_ANALYTICS.BI.DIM_DATE (
  ...
  FISCAL_QUARTER  NUMBER,       -- Vertiv fiscal calendar (same as calendar for now)
  FISCAL_YEAR     NUMBER
)
;

-- Populate DIM_DATE for 2015-2030 (~5,479 rows)
INSERT INTO VERTIV_ANALYTICS.BI.DIM_DATE
WITH DATE_SPINE AS (
...
CREATE OR REPLACE TABLE VERTIV_ANALYTICS.BI.DIM_REGION (
...
INSERT INTO VERTIV_ANALYTICS.BI.DIM_REGION
...
CREATE OR REPLACE TABLE VERTIV_ANALYTICS.BI.DIM_PRODUCT (
...
-- Seed Vertiv product catalog
INSERT INTO VERTIV_ANALYTICS.BI.DIM_PRODUCT
...
FROM (
  ...
  SELECT 'RACK-VR-42U','Vertiv VR 42U Rack','RACK','Vertiv VR','EA',3500,FALSE,'2020-01-01' UNION ALL
  SELECT 'RACK-VR-48U-OF','Vertiv VR 48U Rack','RACK','Vertiv VR','EA',4200,FALSE,'2020-01-01' UNION ALL
...
CREATE OR REPLACE TABLE VERTIV_ANALYTICS.BI.DIM_CUSTOMER (
...
CREATE OR REPLACE DYNAMIC TABLE VERTIV_ANALYTICS.BI.FACT_SALES_ORDERS
...
FROM VERTIV_CURATED.SALES.SALES_ORDER so
...
CREATE OR REPLACE VIEW VERTIV_ANALYTICS.BI.MV_REGIONAL_SALES_DAILY AS
SELECT
  SOURCE_SYSTEM,
  SOURCE_REGION,
  DATE_KEY,
...
FROM VERTIV_ANALYTICS.BI.FACT_SALES_ORDERS
GROUP BY 1,2,3,4;

CREATE OR REPLACE VIEW VERTIV_ANALYTICS.BI.MV_PRODUCT_REVENUE_MONTHLY AS
SELECT
  PRODUCT_SK,
  SOURCE_SYSTEM,
  DATE_KEY,
...
ALTER TABLE VERTIV_ANALYTICS.BI.DIM_CUSTOMER
  ADD SEARCH OPTIMIZATION ON EQUALITY(CUSTOMER_NAME, COUNTRY_CODE, MASTER_CUSTOMER_HK);

ALTER TABLE VERTIV_ANALYTICS.BI.DIM_PRODUCT
  ADD SEARCH OPTIMIZATION ON EQUALITY(PRODUCT_CODE, PRODUCT_NAME, PRODUCT_FAMILY);

-- ── GRANTS ────────────────────────────────────────────────────
GRANT SELECT ON ALL TABLES IN SCHEMA VERTIV_ANALYTICS.BI           TO ROLE VERTIV_GLOBAL_ANALYST;
GRANT SELECT ON ALL TABLES IN SCHEMA VERTIV_ANALYTICS.BI           TO ROLE VERTIV_FINANCE_ANALYST;
GRANT SELECT ON ALL TABLES IN SCHEMA VERTIV_ANALYTICS.BI           TO ROLE VERTIV_EXEC_VP;
GRANT SELECT ON MATERIALIZED VIEW VERTIV_ANALYTICS.BI.MV_REGIONAL_SALES_DAILY    TO ROLE VERTIV_GLOBAL_ANALYST;
GRANT SELECT ON MATERIALIZED VIEW VERTIV_ANALYTICS.BI.MV_PRODUCT_REVENUE_MONTHLY TO ROLE VERTIV_GLOBAL_ANALYST;
