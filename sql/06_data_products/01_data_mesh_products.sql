...
USE DATABASE PLATFORM_CONFIG;

CREATE OR REPLACE TABLE PLATFORM_CONFIG.METADATA.DATA_PRODUCT_CATALOG (
  ...
  SNOWFLAKE_DATABASE    VARCHAR(100)   DEFAULT 'DATA_PRODUCTS',
...
INSERT INTO PLATFORM_CONFIG.METADATA.DATA_PRODUCT_CATALOG VALUES
...
('DP-001',
 ...
 'DATA_PRODUCTS', 'SALES_PERFORMANCE',
 'DATA_PRODUCTS.SALES_PERFORMANCE.SALES_ORDERS_V1',
 'SALES_PERFORMANCE_SHARE',
 'Silver: ENTERPRISE_CURATED.SALES.SALES_ORDER (SAP+JDE+QAD) | Gold: FACT_SALES_ORDERS Dynamic Table',
 ...
 'INTERNAL', 'GLOBAL_ANALYST', FALSE, FALSE,
...
('DP-002',
 ...
 'DATA_PRODUCTS.CUSTOMER_360.CUSTOMERS_V1',
 'CUSTOMER_360_SHARE',
 'Silver: ENTERPRISE_CURATED.SALES.CUSTOMER | Gold: DIM_CUSTOMER, CUSTOMER_REVENUE_FEATURES, MODEL_PREDICTIONS',
 ...
 'INTERNAL', 'GLOBAL_ANALYST', FALSE, TRUE,
...
('DP-003',
 ...
 'DATA_PRODUCTS', 'REVENUE_ANALYTICS',
 'DATA_PRODUCTS.REVENUE_ANALYTICS.MONTHLY_REVENUE_V1',
 ...
 'INTERNAL', 'FINANCE_ANALYST', FALSE, FALSE,
...
('DP-004',
 ...
 'DATA_PRODUCTS', 'OPPORTUNITY_SIGNALS',
 'DATA_PRODUCTS.OPPORTUNITY_SIGNALS.PREDICTIONS_V1',
 NULL,
...
CREATE OR REPLACE TABLE PLATFORM_CONFIG.METADATA.PRODUCT_SLI_LOG (
...
CREATE OR REPLACE VIEW PLATFORM_CONFIG.METADATA.V_PRODUCT_TRUST_SCORE AS
SELECT
  c.PRODUCT_ID,
  c.PRODUCT_NAME,
  c.DOMAIN_NAME,
...
JOIN (
  ...
  SELECT * FROM PLATFORM_CONFIG.METADATA.PRODUCT_SLI_LOG
  WHERE (PRODUCT_ID, MEASUREMENT_TIME) IN (
    SELECT PRODUCT_ID, MAX(MEASUREMENT_TIME)
    FROM PLATFORM_CONFIG.METADATA.PRODUCT_SLI_LOG
    GROUP BY PRODUCT_ID
  )
...
CREATE OR REPLACE TABLE PLATFORM_CONFIG.METADATA.PRODUCT_OUTPUT_CONTRACT (
...
INSERT INTO PLATFORM_CONFIG.METADATA.PRODUCT_OUTPUT_CONTRACT
...
CREATE OR REPLACE VIEW PLATFORM_CONFIG.METADATA.V_DATA_MARKETPLACE AS
SELECT
  c.PRODUCT_ID,
  c.PRODUCT_NAME,
  c.PRODUCT_VERSION,
...
SELECT
  ...
  DESCRIPTION,
  EXAMPLE_VALUE
FROM PLATFORM_CONFIG.METADATA.PRODUCT_OUTPUT_CONTRACT
...
SELECT
  ...
  DATA_AGE_MINUTES,
  FRESHNESS_MET
FROM PLATFORM_CONFIG.METADATA.V_PRODUCT_TRUST_SCORE
ORDER BY TRUST_SCORE DESC;
...
-- ALTER SHARE SALES_PERFORMANCE_SHARE ADD ACCOUNTS = <consumer_snowflake_account>;

-- ════════════════════════════════════════════════════════════════
-- 7. GRANTS — Analysts can query marketplace and trust scores
-- ════════════════════════════════════════════════════════════════
GRANT SELECT ON PLATFORM_CONFIG.METADATA.DATA_PRODUCT_CATALOG    TO ROLE GLOBAL_ANALYST;
GRANT SELECT ON PLATFORM_CONFIG.METADATA.PRODUCT_OUTPUT_CONTRACT TO ROLE GLOBAL_ANALYST;
GRANT SELECT ON PLATFORM_CONFIG.METADATA.PRODUCT_SLI_LOG         TO ROLE GLOBAL_ANALYST;
GRANT SELECT ON PLATFORM_CONFIG.METADATA.V_DATA_MARKETPLACE      TO ROLE GLOBAL_ANALYST;
GRANT SELECT ON PLATFORM_CONFIG.METADATA.V_PRODUCT_TRUST_SCORE   TO ROLE GLOBAL_ANALYST;
GRANT SELECT ON PLATFORM_CONFIG.METADATA.V_DATA_MARKETPLACE      TO ROLE ENTERPRISE_DATA_PRODUCT_OWNER;
GRANT SELECT ON PLATFORM_CONFIG.METADATA.V_PRODUCT_TRUST_SCORE   TO ROLE ENTERPRISE_DATA_PRODUCT_OWNER;
